...
import { alice, betty, bob, constructDao, deployer, fred, passProposalByCoreVote, tom, wallace } from '../helpers';
...
describe('initial distribution', () => {
	...
	it('cannot claim before cliff', async () => {
		constructDao(simnet);
		await simnet.mineEmptyBlocks(105120 / 2);
		checkIsRecipient(alice, 300000000000, 105120, 0);
	...
	it('can claim after cliff', async () => {
		constructDao(simnet);
		checkIsRecipient(alice, 300000000000, 105120, 0);
		await simnet.mineEmptyBlocks(105120 + 1);
		let response = await makeClaim(alice);
	...
	it('can claim incrementally after cliff', async () => {
		constructDao(simnet);
		checkIsRecipient(alice, 300000000000, 105120, 0);
		await simnet.mineEmptyBlocks(105120 / 2 + 1);
		let response = await makeClaim(alice);
		expect(response.result).toEqual(Cl.ok(Cl.uint(150002853881)));
		await simnet.mineEmptyBlocks(105120 / 4);
